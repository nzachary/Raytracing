...
void frame::write_to_file_bmp(std::string path) const {
	...
	for (int i = 0; i < height(); i++) {
		for (int j = 0; j < width(); j++) {
			const color &pixel = get_pixel(j, i);
...
void frame::overwrite_bytes(std::vector<uint8_t> &stream, uint8_t data, size_t offset) const {
	stream[offset] = data;
}

void frame::overwrite_bytes_be(std::vector<uint8_t>& stream, uint16_t data, size_t offset) const {
	stream[offset + 1] = (uint8_t)((data & 0x00FFu) >> 0);
	stream[offset + 0] = (uint8_t)((data & 0xFF00u) >> 8);
}

void frame::overwrite_bytes_be(std::vector<uint8_t> &stream, uint32_t data, size_t offset) const {
...
void frame::overwrite_bytes_be(std::vector<uint8_t>& stream, int32_t data, size_t offset) const {
	stream[offset + 3] = (uint8_t)((data & 0x000000FFu) >>  0);
	stream[offset + 2] = (uint8_t)((data & 0x0000FF00u) >>  8);
	stream[offset + 1] = (uint8_t)((data & 0x00FF0000u) >> 16);
	stream[offset + 0] = (uint8_t)((data & 0xFF000000u) >> 24);
...
void frame::overwrite_bytes_le(std::vector<uint8_t>& stream, uint16_t data, size_t offset) const {
	stream[offset + 0] = (uint8_t)((data & 0x00FFu) >> 0);
	stream[offset + 1] = (uint8_t)((data & 0xFF00u) >> 8);
}

void frame::overwrite_bytes_le(std::vector<uint8_t>& stream, uint32_t data, size_t offset) const {
...
void frame::overwrite_bytes_le(std::vector<uint8_t>& stream, int32_t data, size_t offset) const {
	stream[offset + 0] = (uint8_t)((data & 0x000000FFu) >>  0);
	stream[offset + 1] = (uint8_t)((data & 0x0000FF00u) >>  8);
	stream[offset + 2] = (uint8_t)((data & 0x00FF0000u) >> 16);
	stream[offset + 3] = (uint8_t)((data & 0xFF000000u) >> 24);
